...
fcx %>%
  ...
  filter(!is.na(p)) %>%
  mutate(p.adj = p.adjust(p, method = 'fdr'), .after = p) %>%
  write_csv('results/gene-tpm-FCx-full.csv')
hp %>%
  mutate(p = .hp_p, fold = .hp_f, .after = gene_name) %>%
  filter(!is.na(p)) %>%
  mutate(p.adj = p.adjust(p, method = 'fdr'), .after = p) %>%
  write_csv('results/gene-tpm-Hp-full.csv')

full_join(
  read_csv('results/gene-tpm-FCx-full.csv'),
  read_csv('results/gene-tpm-Hp-full.csv'),
  by = c('gene_id', 'gene_name'),
  suffix = c('_fcx', '_hp')
) %>%
  select(gene_id, gene_name, ends_with('fcx'), ends_with('hp'), everything()) %>%
  write_csv('results/gene-tpm-FCx-Hp-full.csv')
```
